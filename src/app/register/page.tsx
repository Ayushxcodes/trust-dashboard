...
export default function RegisterPage() {
  ...
  // Background API Verification for Professional Memberships
  useEffect(() => {
    if (clientType === "professional" && membershipNumber.trim().length >= 5) {
      setIsCheckingMembership(true);
      setError(null);
      const timer = setTimeout(() => {
        setIsCheckingMembership(false);
        // Simple mock lookup validation
        if (membershipNumber.toUpperCase().startsWith("FCS-") || membershipNumber.toUpperCase().startsWith("ACS-") || membershipNumber.toUpperCase().startsWith("FCA-") || membershipNumber.toUpperCase().startsWith("ACA-") || membershipNumber.length >= 6) {
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 p-1 bg-zinc-100 rounded border border-zinc-200">
                    <button
                      type="button"
                      onClick={() => { setClientType("corporate"); setError(null); }}
                    ...
                    <button
                      type="button"
                      onClick={() => { setClientType("professional"); setError(null); }}
                      className={`py-2 text-center text-xs font-extrabold rounded uppercase tracking-wider transition-all cursor-pointer ${
